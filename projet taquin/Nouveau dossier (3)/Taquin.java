...
public class Taquin {
	...
	public boolean pariteCaseVide (){
		...
		if (positionZero%2==0){
			System.out.println("zero = pair");  //test
		}else{
			System.out.println("zero = impair");  //test
		}
		
		return ((positionZero%2)==0);
	}
	public void pariteNombreDeplacementExp (){
		afficher();
		int nbDeplacement=0;
		for (int i=0;i<hauteur;i++){
			for(int j=0;j<largeur;j++){
					...
					for (int x=0;x<hauteur;x++){
						for(int y=0;y<largeur;y++){
							if(grille[x][y]==(i*largeur)+j){
								System.out.println("on remplace "+grille[i][j]+" avec "+(i*largeur+j)); //test
		...
		System.out.println("nombre de deplacement = "+nbDeplacement);//test
	}
